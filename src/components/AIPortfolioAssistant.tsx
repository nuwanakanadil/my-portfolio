...
const examplePrompts = [
  ...
  'Tell me about the private projects.',
];
...
const projectStats = getProjectStats(manualProjects);
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="max-w-2xl">
                ...
                <p className="mt-3 text-gray-400 leading-7 max-w-2xl">
                  Answers are based on my portfolio, GitHub projects, and learning journey. Public GitHub repositories are loaded automatically in the Projects section, while manual and private projects are curated separately. The assistant calls a backend API route, so the key stays off the frontend.
                </p>
                <p className="mt-3 inline-flex rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-xs font-mono uppercase tracking-[0.22em] text-primary">
                  {projectStats.total} manual projects, {projectStats.featuredProjects} featured, {projectStats.privateProjects} private
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-background/70 px-4 py-3 text-sm text-gray-400 shadow-xl backdrop-blur-sm max-w-sm">
                ...
                <p className="mt-2 leading-6">
                  Smart responses are generated by the serverless API using only portfolio data. Public GitHub repos are loaded in Projects, while private work stays curated. The browser never sees the OpenRouter key.
                </p>
                    ...
                    {portfolioData.projects.slice(0, 3).map((project) => (
                      <div key={project.title} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                        <div className="flex items-center justify-between gap-2">
                          <p className="font-semibold text-white">{project.displayName || project.title}</p>
                          <span className="text-[11px] uppercase tracking-[0.22em] text-primary/80">{project.visibility}</span>
                        </div>
                        <p className="text-[11px] uppercase tracking-[0.22em] text-gray-500">{project.category}</p>
                        <p className="mt-2 text-sm text-gray-400 leading-6">{project.subtitle}</p>
                        <p className="mt-2 text-sm text-gray-500 leading-6">{project.description}</p>
                      </div>
                    ))}
                  </div>
                </div>
